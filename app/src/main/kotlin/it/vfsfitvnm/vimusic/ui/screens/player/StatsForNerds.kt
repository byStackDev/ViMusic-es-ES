        ...
        Box(
            ...
            Row(
                ...
                Column {
                    ...
                    BasicText(
                        text = format?.itag?.toString() ?: "Desconocido",
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                    BasicText(
                        text = format?.bitrate?.let { "${it / 1000} kbps" } ?: "Desconocido",
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                    BasicText(
                        text = format?.contentLength
                            ?.let { Formatter.formatShortFileSize(context, it) } ?: "Desconocido",
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                    BasicText(
                    ...
                    BasicText(
                        text = format?.loudnessDb?.let { "%.2f dB".format(it) } ?: "Desconocido",
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                }
