    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            var appearance by rememberSaveable(
                ...
                with(preferences) {
                    val colorPaletteName = getEnum(colorPaletteNameKey, ColorPaletteName.Dinámico)
                    val colorPaletteMode = getEnum(colorPaletteModeKey, ColorPaletteMode.Sistema)
                    val thumbnailRoundness =
                        getEnum(thumbnailRoundnessKey, ThumbnailRoundness.Ligero)
            ...
            DisposableEffect(binder, isSystemInDarkTheme) {
                ...
                fun setDynamicPalette(colorPaletteMode: ColorPaletteMode) {
                    val isDark =
                        colorPaletteMode == ColorPaletteMode.Oscuro || (colorPaletteMode == ColorPaletteMode.Sistema && isSystemInDarkTheme)

                    binder?.setBitmapListener { bitmap: Bitmap? ->
                        if (bitmap == null) {
                            val colorPalette =
                                colorPaletteOf(
                                    ColorPaletteName.Dinámico,
                                    colorPaletteMode,
                                    isSystemInDarkTheme
                                )
                        ...
                        when (key) {
                            colorPaletteNameKey, colorPaletteModeKey -> {
                                ...
                                val colorPaletteMode =
                                    sharedPreferences.getEnum(
                                        colorPaletteModeKey,
                                        ColorPaletteMode.Sistema
                                    )

                                if (colorPaletteName == ColorPaletteName.Dinámico) {
                            ...
                            thumbnailRoundnessKey -> {
                                val thumbnailRoundness =
                                    sharedPreferences.getEnum(key, ThumbnailRoundness.Ligero)
                ...
                with(preferences) {
                    ...
                    val colorPaletteName = getEnum(colorPaletteNameKey, ColorPaletteName.Dinámico)
                    if (colorPaletteName == ColorPaletteName.Dinámico) {
                        setDynamicPalette(getEnum(colorPaletteModeKey, ColorPaletteMode.Sistema))
                    }
